...
cat.thickness_array = np.linspace(1, 1000, 100) * 1.e-6
...
#    cat.A_arr = cat.A_arr_array[i] ??? !!! consider presenting plots
#    based on this line also
...
for i in range(cat.Kn_length_array.size):
    plt.plot(cat.thickness_array * 1.e6, cat.eta2d[:,i] * 100., MARKERS[i])
#             , label=LABELS[i])
